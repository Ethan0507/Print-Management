	...
	<body>
		...
		<header>
			...
			<div class = 'add_j' id='jform' style="width: 300px; position: fixed; top: 20%; right: 20px;">
				<form class='a_job' method='POST' enctype="multipart/form-data">
					...
					<input type='file' name='ufile' placeholder='' required><br><br>

					<label for='cost'><b>No. of Pages<b></label><br>
					<input type='text' name='pages' placeholder='Enter no. of pages' required><br><br>
					
					<label for='cost'><b>No. of Copies<b></label><br>
					<input type='number' name='copies' placeholder='Enter no. of copies' required><br><br>
		...
		<?php 
			...
			if (isset($_POST['submit']))
			{
				...
				$file_name = rand(100,1000)."-".$_FILES['ufile']['name'];
				...
				else
				{
					if ( $cost <= $GLOBALS['ob'] )
					{
						$t = "insert into jobs values ('$uname','$jname','$exp','$cost','not approved','$file_name')";
						mysqli_query($conn , $t);
						$newb = $GLOBALS['ob'] - $cost;
						$rec = "update registrations set bal = '$newb' where uname = '$uname' ";
						$c = mysqli_query($conn , $rec);
						echo '<script language="javascript">';
						echo 'alert("The cost for the Job is: '.$cost.'\nJob added successfully")';
						echo '</script>';
						header("Refresh:0");
					}
					else
